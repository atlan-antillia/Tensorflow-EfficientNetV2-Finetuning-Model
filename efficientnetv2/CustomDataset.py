  ...
  def create(self, FLAGS):
    ...
    if data_augmentation:
       ...
       train_datagen = tf.keras.preprocessing.image.ImageDataGenerator(
          ...
          width_shift_range  = parser.get(TRAINING, "width_shift_range", 0.2), 
          height_shift_range = parser.get(TRAINING, "height_shift_range", 0.2),
          shear_range        = parser.get(TRAINING, "shear_range", 0.1), 
          zoom_range         = parser.get(TRAINING, "zoom_range", 0.1), 
          #2024/01/11
          brightness_range   = parser.get(TRAINING, "brightness_range", None),  # default=None
          channel_shift_range= parser.get(TRAINING, "channel_shift_range",0.0), # default=0.0
          fill_mode          = parser.get(TRAINING, "fill_mode", "nearest"),    # default="nearest"
          )
