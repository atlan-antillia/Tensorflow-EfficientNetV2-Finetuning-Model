...
class EfficientNetV2ModelTrainer:
  ...
  def save_train_params(self, argv, dir, section="train"):
    # 2023/01/12 Modified to copy 1_train.bat file to model_dir.
    cwd = os.getcwd()
    print("=== current_dir {}".format(cwd))
    batfile = os.path.join(cwd, "1_train.bat")
    if os.path.exists(batfile):
      shutil.copy2(batfile, dir)
  ...
  def build_optimizer(self, learning_rate,
    ...
    if optimizer_name == 'sgd':
      ...
      clipvalue = FLAGS.clipvalue  # 0.5
      print("--- FLAGS.clipvalue {}".format(clipvalue))
